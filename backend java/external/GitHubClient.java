...
public class GitHubClient {
	...
	private List<Item> getItemList(JSONArray array){
		...
		List<List<String>>keywords = getKeywordsFromJsonArray(array);
	...
	public List<List<String>> getKeywordsFromJsonArray (JSONArray array){
		List<String> descriptionList = new ArrayList<>();
		for (int i = 0; i < array.length(); i++) {
			// to extract keywords from description
			//GitHub API doesn't return keywords
			String description = getStringFieldOrEmpty(array.getJSONObject(i), "description");
			if (description.equals("") || description.equals("\n")) {
				//if description has nothing, use "title" to replace description
				descriptionList.add(getStringFieldOrEmpty(array.getJSONObject(i), "title"));
			} else {
				descriptionList.add(description);
			}	
		}
		// We need to get keywords from multiple text in one request
		//because MonkeyLearnAPI has limitations on request per minute.
